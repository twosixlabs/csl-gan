...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=Text, default="/persist/datasets/celeba/img_align_celeba/all/",
        help="<Optional> (Text) Directory containing dataset.", required=False)
    parser.add_argument("--labels_dir", type=Text, default="/persist/datasets/celeba/Anno/list_attr_celeba.txt",
        help="<Optional> (Text) Path to annotations file (for CelebA only)", required=False)
    ...
    for checkpoint in checkpoints:
        ...
        if args.generate_samples:
            ...
            with torch.no_grad():
                ...
                generated_images = []
                for zi, yi in zip(z.split(args.batch_size), [None]*true_num_generated_samples if y is None else y.split(args.batch_size)):
                    generated_images.append(G(zi, yi).cpu())
                generated_images = torch.cat(generated_images, dim=0)

                # generated_images = G(z, y).cpu() REPLACED
